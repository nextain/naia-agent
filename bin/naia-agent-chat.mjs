...
async function doLogin(args) {
  ...
  process.stderr.write(`✓ ${args.provider} 키 저장 → ${ENV_PATH} (${envName}, 0600)\n  이제 'naia-agent-chat' 로 대화하세요.\n`);
  process.exit(0);
}
...
async function doChat(args) {
  ...
  if (args.once) {
    repl.setOnTurnEnd((info) => { shutdown(info.kind === "error" ? 1 : 0); });
    if (!repl.submit(args.once)) { process.stderr.write("빈 메시지.\n"); await shutdown(64); }
    return;
  }
