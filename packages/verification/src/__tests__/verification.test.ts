...
describe("runShellVerifier — exit code + tail capture", () => {
  it("captures stdout tail of node --eval echo", async () => {
    const r = await runShellVerifier(
      process.execPath,
      ["--eval", "process.stdout.write('hi\\n')"],
      os.tmpdir(),
      ctx().signal,
    );
...
describe("Verifiers — TestVerifier / TypeCheckVerifier / LintVerifier / BuildVerifier", () => {
  it.skipIf(process.platform === "win32")("TestVerifier reports exit 0 → pass:true (parseVitestStats unit-tested separately)", async () => {
  ...
  it.skipIf(process.platform === "win32")("TestVerifier reports exit non-zero → pass:false", async () => {
  ...
  it.skipIf(process.platform === "win32")("TypeCheckVerifier passes on exit 0", async () => {
  ...
  it.skipIf(process.platform === "win32")("TypeCheckVerifier fails on non-zero exit", async () => {
    const v = new TypeCheckVerifier({ command: "/bin/false" });
    const r = await v.run("/tmp", ctx());
    expect(r.pass).toBe(false);
  });

  it.skipIf(process.platform === "win32")("LintVerifier passes on exit 0", async () => {
    const v = new LintVerifier({ command: "/bin/true" });
    const r = await v.run("/tmp", ctx());
    expect(r.pass).toBe(true);
  });

  it.skipIf(process.platform === "win32")("BuildVerifier passes on exit 0", async () => {
...
describe("VerificationOrchestrator — parallel + 3중 방어", () => {
  it.skipIf(process.platform === "win32")("runs verifiers in parallel and returns all results", async () => {
    const verifiers: Verifier[] = [
      new TestVerifier({ command: "/bin/true" }),
  ...
  it.skipIf(process.platform === "win32")("never throws on individual runner failure (returns FAIL)", async () => {
    const verifiers: Verifier[] = [
  ...
  it.skipIf(process.platform === "win32")("D27 wall-clock timeout — partial result (pass:false, partial:true)", async () => {
