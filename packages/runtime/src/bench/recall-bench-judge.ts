 ...
 * anchor #3 / §B02: naia-memory does retrieval-only bench; NL / dialogue
 * judgment belongs to naia-agent; ko-serve boundary = serving bench only).
 *
 * Reuses naia-memory's benchmark *pattern*, not its code (Interfaces, not
 * dependencies):
 *  - koIncludes / koNormalizeForJudge — faithful port of naia-memory
 *    src/benchmark/comparison/ko-judge-helpers.ts (deterministic keyword
 *    judge; anchor #8: no external cloud LLM in the judge path).
 *  - TierGate { structureGate, accuracyMin, leakMax } — mirrors
 *    naia-memory src/benchmark/criteria.ts { target, minimum, metric }.
 *
 * User directive (2026-05-20): a tiny model (e2b) is NOT expected to emit a
 * usable marker reliably — for the SMALL tier we only check the structure
 * *can occur at all* (capability gate; low rate is fine, accuracy/leak are
 * reported but NOT gated). Strictness increases with model size: the MID
 * tier (e4b) additionally gates round-trip accuracy + raw-marker leak.
 *
 * Pure module — no model, no I/O. Unit-tested in
 * packages/runtime/src/__tests__/recall-bench-judge.test.ts.
 */

// ── Korean keyword judge (faithful port of naia-memory ko-judge-helpers) ──
...
/**
 * A *well-formed* recall marker (the #41 v2 contract the model is asked to
 * emit). Matched against the model's raw TEXT-channel output (pre-strip),
 * which is exactly what the agent's marker parser at agent.ts:250 can act
 * on. Bound is {2,256} for exact parity with agent.ts:252 (recallQuery
 * must be length >= 2 to drive a recall) — a 1-char marker is inert in
 * production so it must not count as a capable structure here.
 */
 ...
 * Step-3 failure where a tiny model emits a MALFORMED marker (e.g.
...
// ── Tiered gate (criteria.ts pattern) ────────────────────────────────────
...
/**
 * SMALL (e2b) = capability only: prove the structure can occur ≥1 time.
 * Accuracy/leak reported but NOT gated (user: "확률은 당연히 떨어질거고").
 * MID (e4b) = stricter: also gate round-trip accuracy + leak rate.
 * (No `large` tier — #41 v2 scope is e2b/e4b; add one only when a concrete
 * bigger-tier model exists, with its own unit test. Karpathy §2.)
 */
