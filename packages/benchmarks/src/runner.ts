...
interface RunOptions {
	...
	/** R8: optional prepareCompact-style factory closures for pi/hermes.
	 *   Caller (mini-bench-judge.ts) creates one per fixture run so
	 *   anchored-iterative state (previousSummary) is scoped to the fixture. */
	readonly piPrepare?: (
...
export async function runFixture(
	...
	for (let i = 0; i < fixture.turns.length; i++) {
		...
		// R8 (#56) — pi/hermes LLM compaction paths. Same trigger as the
		// in-house path (compactionPoint), but the recap is generated by the
		// host-injected adapter calling its own LLM (bench: GLM HTTP wrapper).
		if (
			compactionPoints.includes(turnIdx) &&
			(strategy === "pi" || strategy === "hermes")
		) {
			...
			const cT0 = performance.now();
			try {
				// Pull options.adapter (caller-supplied factory closure) so the
				// previousSummary state persists across compactionPoints in this
				// fixture run. Initialized lazily on first call.
				const prepare = strategy === "pi" ? opts.piPrepare : opts.hermesPrepare;
				if (!prepare) {
					throw new Error(
						`runFixture(${strategy}): options.${strategy}Prepare not provided`,
					);
				}
				const pruned = await prepare(llmHistory);
				compactionLatencies.push(performance.now() - cT0);
				if (pruned && pruned.length > 0) {
					// Find the inserted summary message — `[Compacted summary` or
					// `[Hermes compacted summary` prefix. Save as recapContent so
					// evaluateProbe + visible-context builder see the recap.
					const summaryMsg = pruned.find(
						(m: LLMMessage) =>
							typeof m.content === "string" &&
							(m.content.startsWith("[Compacted summary") ||
								m.content.startsWith("[Hermes compacted summary")),
					);
					if (summaryMsg && typeof summaryMsg.content === "string") {
						// Strip the prefix bracket so recapContent is just the body.
						const idx = summaryMsg.content.indexOf("]\n\n");
						recapContent = idx >= 0
							? summaryMsg.content.slice(idx + 3)
							: summaryMsg.content;
					} else {
						recapContent = "";
					}
				...
				compactionLatencies.push(performance.now() - cT0);
				errors.push(`${strategy}@${turnIdx}: ${String(err)}`);
				recapContent = "";
			}
		}
