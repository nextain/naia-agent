/**
 * 5-axis metric collectors — Slice 3-XR-Compact P1 skeleton.
 *
 ...
 * or absolute milliseconds for latency. Full LLM-judge wiring lands in P6.
 *
 * Design notes:
 * - task-accuracy uses LLM-judge ensemble for `task-accuracy` probes only.
 *   `fact-recall` falls back to string-match → LLM-judge only on miss.
 ...
 *   semantic divergence (1 - cosine on response embeddings, then LLM-judge
 *   tie-break in the 0.6 < d < 0.85 ambiguous band).
 */
...
/**
 * Drift between two responses. P1 stub: token-overlap Jaccard as cheap proxy.
 * P6 replaces with cosine-on-embeddings + LLM-judge tie-break.
 */
