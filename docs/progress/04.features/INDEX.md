...
| SPEC-014 | UC-017, UC-020 | **역할·도구 위임** — app-server `dynamicTools`/`item/tool/call` 즉시 응답, provider-native handled event, 승인·외부처리 도구 제외, desktop/Discord `delegate_agent`의 Pi `expert/main/sub` allowlist+동적 설정 reload+realpath 검증, 역할별 `sub_llm` 처리정책, Discord 직렬 진행 egress | agent | Done | TEST-F-014 |
...
| SPEC-020 | UC-024 | **Naia coding-session tool bridge** — trusted runtime binding을 기존 MultiIssueSessionManager 명령/조회에 연결하고 safe projection만 반환; optional CLI coding config가 기존 chat 도구와 합성 | agent CLI | In-progress | TEST-F-020 |
