...
| SPEC-012 | UC-015 | **같은 요청 연속 발화 제어** — `ChatTurnHandler` 내장 `continue_speaking` semantic tool + provider-local assistant/숨은 진행 지시 threading + abort 가능한 발화 간격 + 시간/횟수 상한. 기존 gRPC stream/TTS/cancel 재사용, usage·terminal·기록 1턴 불변식 보존 | agent | Approved | TEST-F-012 |
