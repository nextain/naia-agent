...
| SPEC-007 | UC-012 | **budgeted conversation assembly** — ConversationPort 실구현(char≈token 휴리스틱 예산, systemPrompt 보존, 오래된 메시지 절단, **tool 라운드 원자 블록**=assistant+tool 동시 보존, **toolCalls payload 예산 계산**, 선두 고아 tool 가드, 최신 블록 보존). `adapters/budgeted-conversation.ts` — passthrough 스텁 교체 | agent | Done | TEST-F-007 |
