...
| REQ-015 | 역할·도구 위임 | 로컬 Codex 로그인 기반 main provider가 자동승인 로컬 도구를 app-server 동적 도구 RPC로 실행하고, 데스크톱/Discord host가 워크스페이스에 고정된 Pi `expert/main/sub` 역할 위임, 역할 설정 reload, `sub_llm` 처리정책과 비밀 없는 진행 이벤트를 제공 | Done | UC-017, UC-020 | SPEC-014 | TEST-S-017 |
...
| REQ-025 | Naia 코딩 총괄 비서 | persona·memory·KB 대화와 durable coding-session start/list/show/answer/cancel을 한 Naia CLI에서 결선하고 Codex/Claude Code/OpenCode/Pi를 선언형 작업자로 유지 | In-progress | UC-024 | SPEC-020 | TEST-S-024 |
