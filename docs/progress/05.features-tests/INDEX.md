...
| TEST-F-012 | SPEC-012 | 연속 발화 제어 — 명시 요청 quote 검증·순차 provider 호출·이전 발화 context·제어 도구 재노출 방지·발화 사이/호출 중 100ms 취소와 iterator/timer/listener 정리·취소 save 0·시간/횟수 상한·인자 no-throw 정규화·usage→terminal 1회·정상 transcript/memory 1턴·일반/혼합 외부 도구 승인거부·timeout·correlation 무회귀 | `src/test/uc-continue-speaking.contract.test.ts` | Planned |
