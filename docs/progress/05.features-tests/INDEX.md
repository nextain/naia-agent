...
| TEST-F-014 | SPEC-014 | app-server 현재 스키마의 동적 도구 광고·server request 응답, native tool event 단일 실행, desktop Codex 위임 allowlist·realpath 탈출 방지, Discord 진행 이벤트의 안전 이름·직렬 순서·final dedupe 보존 | `src/test/codex-app-server-provider.contract.test.ts`, `uc1-agent.contract.test.ts`, `delegate-agent-skill.contract.test.ts`, `discord-entry-wiring.contract.test.ts`, `discord-runtime.integration.test.ts` | Pass |
