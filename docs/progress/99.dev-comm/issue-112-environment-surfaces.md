...
- 2026-06-30 `d8d07dd` — 이 저장소가 화이트리스트를 `avatarEmotion | panel | responseStyle` 로 고정
- 2026-07-01 `8d51b57a` — naia-shell 이 "panel→app 리팩터" 로 **자기 쪽 이름만** 바꿈
...
디코더 경계에서 `app` 을 `panel` 별칭으로 받아 정본 이름으로 눕히고 회귀 테스트 5건을 붙였다.
자세한 내용과 남은 결정은 #113.
