...
| TEST-S-015 | UC-015 | 명시 요청 quote 검증→내부 제어 도구 활성화→같은 requestId 순차 복수 발화, context 연속성, 100ms 취소·자원정리, 시간/횟수 상한, 인자 정규화, usage→terminal/기록 불변식(취소 save 0), 일반 채팅·승인거부/timeout 포함 일반 도구 무회귀 | 계약+실연동 | `src/test/uc-continue-speaking.contract.test.ts` + `src/test/uc-continue-speaking.ollama.integration.test.ts` 고정 프롬프트 2회 | Planned |
