...
| TEST-S-016 | UC-016 | 허용목록 ingress, 두 채널 겹침·역순 완료 라우팅/기록 격리, reconnect/rate-limit/cancel, durable dedupe, lifecycle epoch·authority를 fake Gateway로 검증. 실제 bot 2채널 송수신·RESUME·403·rotate/revoke는 운영자 인수 대기 | seam통합 + live | `src/test/discord-runtime.integration.test.ts`, `discord-gateway.contract.test.ts`, `discord-live.integration.test.ts` | Partial |
