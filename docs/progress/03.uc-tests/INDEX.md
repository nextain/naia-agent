...
| TEST-S-017 | UC-017 | Codex app-server experimental dynamicTools와 item/tool/call 응답, 자동승인/외부처리 필터, handled event 재실행 방지, desktop Codex-only+realpath+child OS sandbox 위임, Discord 진행 메시지 무비밀·순서·최종응답 보존을 계약/통합 검증 | 계약+통합+실 smoke | `src/test/codex-app-server-provider.contract.test.ts`, `uc1-agent.contract.test.ts`, `delegate-agent-skill.contract.test.ts`, `uc-cli-subagent-codex.contract.test.ts`, `discord-entry-wiring.contract.test.ts`, `discord-runtime.integration.test.ts` | Pass |
