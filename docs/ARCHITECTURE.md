...
## 5. 흔들림 방지 (CI 게이트)
`check-compile-integrity`(tsc) · `check-logging`(DiagnosticLog 강제) · `check-file-anchors`(`module-manifest.json` — `src/main` 의 모든 `.ts` 가 {layer,uc,contract} 앵커와 1:1 등록, 현재 44 파일) · vitest(`src/test/uc*.contract/integration.test.ts`) — 전부 `.github/workflows/self-trust-gates.yml` `code-gates` job 에서 자동실행. PreToolUse 훅(file-anchor-guard)은 인터랙티브 세션 1차 방어.
