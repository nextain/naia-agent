...
| FR-MEM-15 | 인스턴스 간 기억 오염 방지(#108, naia-shell#425) — 로컬 기억 저장 경로는 agent 가 **명시 해석해 항상 LocalAdapter 에 전달**한다(패키지 기본값 미사용). 해석 = 비어있지 않은 `NAIA_HOME` 이 `~/.naia` 를 대체(격리된 dev 인스턴스 `Naia Dev` 는 `~/.naia-dev/memory/...` 사용, 운영 기억 무접촉). `NAIA_HOME` 미지정 기본값은 기존 패키지 기본과 동일 경로(무회귀). 호출자가 storePath 를 명시하면 그것이 우선. | Done |
