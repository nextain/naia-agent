...
- **순환검증** — 자체 테스트(전 vitest 스위트)는 모두 구현자 작성. 외부 이질 repo corpus 기반 통합테스트 필요.
- **런타임 시크릿 누출(로그)** — message/ctx 에 섞인 평문 키·토큰은 `adapters/redact.ts`(diag sink write-직전)가 **best-effort 마스킹**(sk-/AIza/ghp/xox/AKIA/gw/JWT + 키문맥). 완벽한 DLP 아님 — 1차 방어는 "비밀 값 로그 금지" 규율(`docs/logging.md`).
