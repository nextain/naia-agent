...
| paired cost evidence permits a bounded savings claim only for equal task/call denominators, restored checkpoints, deterministic quality non-inferiority, and exact settled gateway customer-billing receipts; estimates, window aggregates, contamination, route/token drift, or unresolved calls fail closed | `pi-cost-comparison.contract.test.ts`, `benchmark/orchestration/pi-cost-comparison.json` |
