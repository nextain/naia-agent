...
| paired cost evidence permits a bounded savings claim only for equal tasks and actor-attempt topology, restored checkpoints, deterministic quality non-inferiority, and every exact settled gateway customer-billing request; tool-loop request-count differences remain measured rather than being confused with role counts, while estimates, window aggregates, contamination, route/token drift, or unresolved calls fail closed | `pi-cost-comparison.contract.test.ts`, `pi-cost-comparison-runner.contract.test.ts`, `benchmark/orchestration/pi-cost-comparison.json` |
