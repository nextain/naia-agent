...
## 4. Slice 1 — `bin/naia-agent` 최소 REPL

### 4.1 목표

**naia-agent를 처음으로 사용자 가치 있는 도구로 만든다**. `pnpm exec naia-agent "hi"`가 진짜 Anthropic API를 호출하고 답을 출력. 메모리·스킬은 Slice 3+ 까지 mock-substitute.
...
- **G06**: Memory stubs — 일부 해소 (issue #1과 cross-link)
- **E07**: Memory 양방향성 시점 — 결정 강제 (encode 시점 명시)

### 6.6 의존

- alpha-memory 자체의 stub 구현(contentTokens, jaccardSimilarity, mergeRelatedFacts)이 issue #1에서 처리됐는지 확인. 미처리면 본 슬라이스도 stub-aware mode로 동작.
