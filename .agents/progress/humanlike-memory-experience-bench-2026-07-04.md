...
### 다음 (Luke 지시 = Slice 2)
- **Slice 2: social-quality 판정 배선** — `used-needs-judge` 층에 codex+claude 앙상블(적절성·자연스러움·충실성 0~3 median). Gemini는 SUT라 판정 제외. `NAIA_JUDGE_ENSEMBLE`류 게이트로 크레딧 보호. 재사용 = `packages/benchmarks/src/judges/`(cli-judge codex/claude + ensemble).
- Slice 3: 감정연상 2 + 취향 +1(총 4, 부정 probe 포함). Slice 4: report/fixture 녹화(CI 회귀).
- 커밋: HL-1 슬라이스(브랜치 `migration/slice-hl-1-humanlike-mem`) + core 수정 별도 커밋.
