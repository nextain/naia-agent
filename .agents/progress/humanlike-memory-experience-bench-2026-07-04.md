...
### 다음 (Slice 3+)
- **Slice 3: 감정연상 시나리오 2 + 취향 +1(총 4)** — 부정 probe 포함. 감정으로 연결된 과거 회상(화제 전환 시). 취향보다 주관적이라 판정 축 확장 검토.
- Slice 4: report/fixture 녹화(CI 회귀, real-LLM 없이 재생).
- 커밋: HL-1(runner)+core 수정(`migration/slice-hl-1-humanlike-mem`) → HL-2 판정(`migration/slice-hl-2-social-judge`).
- 미해결(사람): 판정 임계값(2.0) 캘리브레이션은 시나리오 늘린 뒤(Slice 3) 데이터로 조정. codex 크레딧 = gated 실행만.
