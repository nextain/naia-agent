...
| **A24** | DANGEROUS_COMMANDS regex catalog (12+ 패턴) | OWASP A03 + CWE-78 (D01 §D → §A, Slice 2). F09 cleanroom 라인 인용 0. **적대적 리뷰 F1 보강(2026-05-18)**: rm-rf 패턴 구분자 `[\s;&|]`→`[\s;&|"']` + 끝 조건 `[/~][\s/]`→`[/~][\s/"']` — `eval "rm -rf /"` 우회 차단 | `packages/runtime/src/utils/dangerous-commands.ts` | 40 unit (dangerous-commands.test.ts — block 19 + allow 16 + assertSafe 2 + 메타 2 +1 eval-quoted-rm-rf) |
...
| **A30** | File ops skills (read_file/write_file/edit_file/list_files) — D09 sentinel 재사용 | 자체 + claude-code/aider 영감 (Slice 2.6). T0 read/list, T1 write/edit. **적대적 리뷰 F2 보강(2026-05-18)**: bundle e2e 테스트 positional 구조분해 → `.find(s => s.name === ...)` name-based lookup으로 교체 (순서 변경 시 조용한 오검증 방지) | `packages/runtime/src/skills/file-ops.ts` + `createFileOpsSkills()` bundle | 23 unit (file-ops.test.ts) + GLM 실 호출 검증 (list_files로 ref review 11개 정확히 출력) |
