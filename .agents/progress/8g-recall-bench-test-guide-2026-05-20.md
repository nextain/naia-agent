...
## 4. 미결 설계 판단 (#41 v2 — 사용자 소유)

개념은 건전(3-a 증명). 8G에서 실동작시키려면 agent.ts/#41 변경 필요.
어느 방향으로 갈지 = 사용자 결정 (별도 질문으로 제시).

- **A. 관대한 파서**: agent.ts 마커 regex가 `<recal>` 등 변형 허용
  (소형모델 오타 흡수). 트레이드=오탐 위험, 재크로스리뷰 필요.
- **B. 프롬프트 합성 수정**: 8G tier 전용 경량 프롬프트 경로로
  직접호출 수준 출력 유도(열화 제거). 가장 근본적.
- **C. 8G recall 정책 변경**: always-on start-of-turn recall 끄고
  마커를 유일 트리거로(사용자 원비전 "큰 뇌가 직접 요청"에 부합).
- **D. 현 상태=문서화된 발견으로 수용**, 8G 마커는 후속 트랙.
