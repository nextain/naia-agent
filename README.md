...
> `@nextain/naia-memory` 의존이 로컬 경로 `file:../../naia-memory`(= new-naia-agent 에서 **두 단계 위**)
> 라서, **new-naia-agent 의 조부모 디렉토리**에 `naia-memory` 가 있어야 `pnpm install` 이 성공한다
...
> #   └── new-naia/new-naia-agent/     ← 여기서 pnpm install
