...
**naia 생태계의 "뇌" 런타임이자 sub-agent 오케스트레이터.** 셸(naia-os)이 메시지를 던지면 agent 가
대화를 조립하고 LLM provider 를 호출하고 도구를 실행해 응답을 스트리밍한다 — 나아가 외부 코딩
에이전트(pi · opencode · claude-code 등)를 **sub-agent 로 spawn·감독**하고, 워크스페이스 변경을
관찰하고 검증(test/lint/build)해 **정직한 숫자 리포트**를 낸다. 즉 단말에서 외부·로컬 LLM 을 잇는
오케스트레이터 클라이언트("Hermes")이자, naia-os 워크스페이스·naia-adk 와 연계해 일하는 에이전트.
...
| **sub-agent 오케스트레이션** | 외부 코딩 에이전트(pi · opencode-cli + roster, claude-code/codex/gemini 선언)를 `SubAgentPort` 로 spawn → 이벤트 스트림 forward + 인터럽트(SIGTERM→유예→SIGKILL). supervisor 가 단일 작업 감독. composition `wireSupervisor` 로 조립 | ✅ |
| **정직보고(workspace+verify)** | `WorkspacePort`(git 변경 요약) + `VerifierPort`(test/lint/build 러너, never-throws) → session_end 후 검증해 filesChanged/검증 결과 숫자 리포트 | ✅ |
...
> **사전 요구: `naia-memory` 를 아래 레이아웃으로 함께 clone.** `package.json` 의
> `@nextain/naia-memory` 의존이 로컬 경로 `file:../../naia-memory`(= naia-agent 에서 **두 단계 위**)
> 라서, **naia-agent 의 조부모 디렉토리**에 `naia-memory` 가 있어야 `pnpm install` 이 성공한다
> (메인테이너 모노레포 구조를 반영 — agent 는 한 단계 더 깊은 하위 폴더 아래 둔다).
>
> ```bash
