...
| **agent-local 스킬** | github · obsidian · 메모 · 날씨(openmeteo) · MCP 브리지 · composite · 승인게이트 · cron · notify(slack/discord) | ✅ |
...
| **브라우저 스킬** | cmd 화이트리스트 + 주입 CLI(navigate/click/fill). 실 CDP 런타임은 호스트머신 defer | ✅ |
| **BGM 스킬** | youtube 검색/재생/볼륨. 실 플레이어 런타임은 호스트머신 defer | ✅ |
| **대화 압축(compaction)** | 긴 대화를 토큰예산 안으로 조립 — 최신 우선·tool 라운드 원자 보존·systemPrompt 보존 | ✅ |
| **장기기억·RAG 연동** | `@nextain/naia-memory` recall/save 배선 | 🔜 진행 중(`feat/memory-wiring`) |
| **레퍼런스 호스트** | naia-os 임베드(✅) · CLI 호스트 · 메신저 봇 | 🔜 일부 계획 |

> ✅ = main 에 구현+계약테스트 통과. 🔜 = 설계됐고 일부 코드 존재, 통합 진행 중.
...
```bash
node dist/main/index.js     # stdout 에 `GRPC_LISTENING <addr>` 출력 → 호스트가 connect
```
