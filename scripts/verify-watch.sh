...
run_checks() {
  ...
  # 게이트 활성 — core.hooksPath 풀리면 pre-commit 무결성 게이트 비활성(로컬설정 fragile). 풀림=RED → setup 재실행 안내.
  if [ "$(git -C "$ROOT_DIR" config core.hooksPath 2>/dev/null)" != "scripts/git-hooks" ]; then
    echo "[hooks-gate] core.hooksPath 미설정 — pre-commit 게이트 비활성. 'bash scripts/setup-git-hooks.sh' 재실행." >>"$WORK_DIR/.vw_tmp" 2>/dev/null || true
  fi
