/**
 * Fixture schema for compaction / recall benchmarks.
 *
 * Self-contained types so this benchmark module does not depend on the agent's
 * internal message shapes. A runner (later stage) adapts FixtureTurn → core
 * LLM message at execution time.
 *
 * Ported faithfully from the pre-rewrite monorepo
 * (`packages/benchmarks/src/fixture.ts`). Stage 1a keeps only the pure schema +
 * validator; no runner / report coupling. The orchestration-independent metrics
 * in `metrics.ts` depend on the `FixtureProbe` type defined here.
 *
 * Source format (JSON files in `../fixtures/`):
 *
 *   {
 *     "id": "F001-customer-support",
 ...
 * compares responses for semantic equivalence.
 */
