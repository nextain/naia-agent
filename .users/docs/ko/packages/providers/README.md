...
| `LabProxyLiveClient` | `./lab-proxy-live` | Naia Lab Gateway WSS, `/v1/realtime` audio_delta 경로 (naia-omni realtime 엔드포인트). |
...
1. **자동 설치 기본 번들** (이 패키지의 `optionalDependencies`, 편의를
   ...
   - `@ai-sdk/openai-compatible` — vLLM / LM Studio / Ollama / OpenRouter / 등
...
pnpm add ai @ai-sdk/openai-compatible      # vLLM / LM Studio / Ollama / OpenRouter / 등
...
| **vLLM / LM Studio / Ollama (OpenAI 호환)** | `@ai-sdk/openai-compatible` | 서버별 | `baseURL` override, `/v1/chat/completions` |
...
- **`LabProxyLiveClient`** — WSS, `/v1/realtime` audio_delta 경로 (naia-omni realtime 엔드포인트).
