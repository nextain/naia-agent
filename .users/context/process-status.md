...
기존 Issue #84 `current_work`를 덮어쓰지 않고, 2026-07-19부터 두 세션을 별도
트랙으로 관리한다.

| 트랙 | 작업 | worktree | 현재 게이트 |
|---|---|---|---|
| Session 1 | `naia-agent#89` 공통 wire 계약 | `projects/naia-agent-worktrees/issue-89-codex-provider` | P01~P05 결정론 검증 완료, guardian PASS + 독립 2-pass CLEAN, 동결 후보 |
| Session 2 | `naia-agent#85/#86` Discord·required-grounding | `projects/naia-agent-worktrees/issue-85-discord-rag-s2` | 정책 slice READY, 동결 commit 소비 후 production 결선 필요 |

Session 1의 권위 계약은
`docs/progress/99.dev-comm/UC-WIRE-V1-contract-2026-07-19.md`다. Session 2는
`[CONTRACT FROZEN v1] naia-agent#89` 전까지 공유 wire/proto/composition을
수정하지 않는다. Session 1 결정론 검증은 Agent 1,191 PASS/8 live skip,
Shell 1,253 PASS/13 skip, paired Rust 검증과 두 번의 독립 CLEAN까지 완료했다.
실제 Discord credential·외부 provider·Qdrant E2E는 동결 후 Session 2 통합과
live credential gate에서 별도로 검증한다.
