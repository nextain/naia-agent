...
- 상태: 구현·통합 테스트 완료, 최종 벤치·2회 Clean 리뷰 진행 중 (`REQ-021 / UC-ORCH-001`)
...
- P05: 최종 정확 리비전의 유료 벤치 증거와 2회 연속 Clean 리뷰를 남긴 뒤 완료한다.
- 범위: 현재 단계는 포트 뒤 라이브러리·조합 및 벤치 경로까지다. 운영 채팅/CLI 진입점과
  naia-shell 연결은 후속 단계이며 현재 동작한다고 주장하지 않는다.
- 기준선: `c424be6`; 빌드는 통과했다. 전체 테스트는 1,426개 통과·10개 건너뜀,
  관련 테스트 56/56 및 두 기존 process 테스트를 제외한 전체 1,451개가 통과했다(최신 집중 사례 2건은
  다음 전체 재실행에서 합산). 기존 실패는 관리
