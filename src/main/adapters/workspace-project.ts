...
export interface WorkspaceIdDeps {
  ...
  /** Existing workspace root의 filesystem canonical path. 실패/불일치는 fail-closed. */
  realpath(path: string): string;
  randomUUID(): string;
}

export interface ProductMigrationDeps {
  exists(path: string): boolean;
  mkdir(path: string): void;
  validateSource(source: string, expectedRoot: string): void;
  copyExclusive(source: string, destination: string): void;
}

function copyLegacyFile(source: string, sourceRoot: string, destination: string, deps: ProductMigrationDeps): boolean {
  if (deps.exists(destination) || !deps.exists(source)) return false;
  deps.validateSource(source, sourceRoot);
  deps.mkdir(dirname(destination));
  try {
    deps.copyExclusive(source, destination);
    return true;
  } catch (error) {
...
export function migrateLegacyMemoryStoreFile(
  adkPath: string,
  legacyStorePath: string,
  deps: ProductMigrationDeps,
): boolean {
  const source = resolve(legacyStorePath);
  return copyLegacyFile(source, dirname(source), resolveProductStorage(adkPath).memoryStorePath, deps);
}

/** Destination-wins compatibility copies. Sources are retained for rollback. */
export function migrateLegacyWorkspaceIdentity(adkPath: string, deps: ProductMigrationDeps): boolean {
  const root = resolve(adkPath);
  return copyLegacyFile(
    join(root, ".naia", "workspace-id"),
    join(root, ".naia"),
    resolveProductStorage(root).workspaceIdPath,
    deps,
  );
}

export function migrateLegacyMemoryStore(
  adkPath: string,
  project: string,
  legacyMemoryRoot: string,
  deps: ProductMigrationDeps,
): boolean {
  const legacyRoot = resolve(legacyMemoryRoot);
  return copyLegacyFile(
    join(legacyRoot, storeDirKey(project), "store.json"),
    legacyRoot,
    resolveProductStorage(adkPath).memoryStorePath,
    deps,
  );
}
...
export function migrateLegacyKnowledge(
  ...
  const root = resolve(adkPath);
  return copyLegacyFile(
    join(root, "knowledge", scope, "kb.json"),
    join(root, "knowledge"),
    join(resolveProductKnowledgeDir(root, scope), "kb.json"),
    deps,
  );
}
...
/** Product persistence is owned by naia-agent and is always rooted in the selected ADK settings tree. */
export function resolveProductStorage(adkPath: string): {
  settingsDir: string;
  memoryDir: string;
  ...
  if (typeof adkPath !== "string" || !adkPath.trim())
    throw new Error("adkPath is required for product storage");
  const settingsDir = join(resolve(adkPath), "naia-settings");
  const memoryDir = join(settingsDir, "memory");
  return {
    settingsDir,
    memoryDir,
    memoryStorePath: join(memoryDir, "store.json"),
    workspaceIdPath: join(memoryDir, "workspace-id"),
  };
}

export function resolveProductKnowledgeDir(adkPath: string, scope: string): string {
  const normalizedScope = scope.normalize("NFC");
  if (!isSafeProductScope(normalizedScope))
    throw new Error("knowledge scope must be a safe path segment");
  return join(resolveProductStorage(adkPath).settingsDir, "knowledge", normalizedScope);
}

/** Shared validator for every producer/consumer of `knowledge/<scope>`. */
export function isSafeProductScope(scope: string): boolean {
  return !(
    typeof scope !== "string" ||
    scope.length === 0 ||
    scope.length > 128 ||
...
/** workspace identity = **워크스페이스에 영속 저장된 UUID**(`<adkPath>/naia-settings/memory/workspace-id`). 경로 해시
 ...
 *  같은 identity 공유(교차누설). 진짜 clone 격리는 OS 관리 레지스트리 필요(미래). 완화=
 *  `naia-settings/memory/workspace-id`
...
export function resolveWorkspaceId(adkPath: string, deps: WorkspaceIdDeps): string {
  ...
  const canonical = resolve(deps.realpath(abs));
  const storage = resolveProductStorage(canonical);
  const dir = storage.memoryDir;
  const idFile = storage.workspaceIdPath;
