...
export function isLocalEngineBaseUrl(baseUrl: string): boolean {
	...
	if (host.endsWith(".ts.net")) return true; // Tailscale MagicDNS — 자가 tailnet 전용 TLD(#118)
	...
	if (a === 100 && b >= 64 && b <= 127) return true; // 100.64.0.0/10  (RFC6598 CGNAT — Tailscale 자가망, #118)
	return false;
}
