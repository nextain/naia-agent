...
describe("UC-ORCH-001 single issue", () => {
  ...
  it("terminalizes an externally aborted in-flight worker as cancelled and preserves its paid receipt", async () => {
    ...
    const orchestrator = new SingleIssueOrchestrator({ ...h.deps, worker: { async execute(input) {
      enteredResolve(); await new Promise<void>((_, reject) => input.signal.addEventListener("abort", () => reject(
    ...
    expect({ roles: orchestrator.snapshot("issue-0001").receipts.map((item) => item.role), event: h.store.events("issue-0001").at(-1)?.type })
      .toEqual({ roles: ["naia", "moderator", "worker"], event: "issue_cancelled" }); h.store.close();
  });
