// UC-APP 계약 — app-tool-executor(환경 도구 위임). FR-APP-1~5.
// E1: agent 는 실행 안 함 — execute()=app_tool_call emit→AppToolResult 대기. fake egress 로 emit 캡처(실 gRPC 무의존).
import { describe, it, expect, vi } from "vitest";
import { makeAppToolExecutor } from "../main/adapters/app-tool-executor.js";
...
describe("app-tool-executor (UC-APP FR-APP)", () => {
  ...
  it("FR-APP-2/3: execute → app_tool_call emit + AppToolResult(requestId+toolCallId) 로 해소", async () => {
    ...
    expect(emitted[0].e).toMatchObject({ kind: "appToolCall", toolCallId: "tc1", toolName: "skill_x", args: { a: 1 } });
    p.resolveResult("req1", "tc1", "done", true);
    await expect(resP).resolves.toEqual({ output: "done", isError: false });
  });

  it("FR-APP-3: success=false → isError:true", async () => {
    const { egress } = fakeEgress();
    const p = makeAppToolExecutor({ egress });
  ...
  it("FR-APP-6: screenshot data URI → bounded inline image, base64 text는 결과에서 제거", async () => {
    const { egress } = fakeEgress();
    const p = makeAppToolExecutor({ egress });
    ...
    await expect(resP).resolves.toEqual({
      output: "App screenshot captured and attached as an inline image.",
      images: [{ mimeType: "image/png", data: ONE_PIXEL_PNG }],
    });
  });

  it("FR-APP-6: MIME signature 불일치 screenshot은 fail closed", async () => {
    const { egress } = fakeEgress();
    const p = makeAppToolExecutor({ egress });
  ...
  it("FR-APP-4: timeout → isError, 늦은 결과는 no-op(누수 0)", async () => {
    vi.useFakeTimers();
    try {
      const { egress } = fakeEgress();
      const p = makeAppToolExecutor({ egress, timeoutMs: 1000 });
  ...
  it("FR-APP-4: abort → isError cancelled, 이후 결과 no-op", async () => {
    const { egress } = fakeEgress();
    const p = makeAppToolExecutor({ egress });
  ...
  it("FR-APP-4: 다중 동시 execute 독립 매칭(역순 해소)", async () => {
    const { egress } = fakeEgress();
    const p = makeAppToolExecutor({ egress });
...
// H1 재발방지: composite 에 동적 app child 를 끼운 통합(entry 와 동일 합성 순서 = app 등록 *전* 합성).
// 단위 격리(위 describe)는 이 갭을 구조적으로 못 잡는다 — composite 가 specs 를 구축시점 스냅샷하면 app 영영 미노출.
describe("composite + 동적 app 통합 (H1 재발방지)", () => {
  const builtin = (): ToolExecutorPort => ({
    specs: () => [{ name: "get_time", description: "time", parameters: {} }],
    execute: async () => ({ output: "now" }),
  });

  it("app 등록 전 composite 합성 → 등록 후 specs/execute 동적 반영 + builtin 무회귀 + clear 동적", async () => {
    const { egress, emitted } = fakeEgress();
    const app = makeAppToolExecutor({ egress });
    const composite = makeCompositeToolExecutor([builtin(), app]); // ★ entry 와 동일: app 등록 전에 합성
    expect(composite.specs().map((s) => s.name)).toEqual(["get_time"]);
    app.register("bgm", [{ name: "skill_youtube_bgm", description: "bgm", parameters: {} }]);
    ...
    expect(emitted).toHaveLength(1);
    expect(emitted[0].e).toMatchObject({ kind: "appToolCall", toolName: "skill_youtube_bgm" }); // 위임 emit
    app.resolveResult("r", "tc", "playing", true);
    expect(await resP).toEqual({ output: "playing", isError: false });
    expect(await composite.execute({ id: "tc2", name: "get_time", args: {} }, { requestId: "r" })).toEqual({ output: "now" }); // FR-APP-5 builtin 무회귀
    app.clear("bgm");
